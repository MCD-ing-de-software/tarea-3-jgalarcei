...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError) as cm:  
            result = cleaner.drop_invalid_rows(df, ["Col_No_Existe"])
        self.assertIn("Columns not found in DataFrame: ['Col_No_Existe']", str(cm.exception))
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        df['name'] = df['name'].fillna('') ## Se reemplazan los None para procesarlo como String ##
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        with self.assertRaises(TypeError) as cm:  
            result = cleaner.trim_strings(df, ["age"])
        self.assertIn("Columns are not string dtype: ['age']", str(cm.exception))
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        self.assertNotIn(float(120.0),result["age"].tolist())
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError) as cm:  
            result = cleaner.remove_outliers_iqr(df, "salary", factor=0.5)
        self.assertIn("Column 'salary' not found in DataFrame", str(cm.exception))
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError) as cm:  
            result = cleaner.remove_outliers_iqr(df, "city", factor=0.5)
        self.assertIn("Column 'city' must be numeric to compute IQR", str(cm.exception)) 
