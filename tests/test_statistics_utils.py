...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        self.assertEqual(result.shape,esperado.shape,"Shape iguales")
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError) as cm:  
            result = utils.moving_average(arr,window=10) 
        self.assertIn("window must not be larger than the array size", str(cm.exception))
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        with self.assertRaises(ValueError) as cm:  
            result = utils.moving_average(arr,window=2) 
        self.assertIn("moving_average only supports 1D sequences", str(cm.exception))
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        self.assertAlmostEqual(result.mean(), 0.0, msg="La media de los z-scores no es aproximadamente cero.")

        self.assertAlmostEqual(result.std(), 1.0, msg="La desviación estándar de los z-scores no es aproximadamente uno.")
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        with self.assertRaises(ValueError) as cm:  
            result = utils.zscore(arr) 
        self.assertIn("Standard deviation is zero; z-scores are undefined", str(cm.exception))
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        self.assertAlmostEqual(result.min(), 0.0, msg="El valor minimo no es cero")

        self.assertAlmostEqual(result.max(), 1.0, msg="El valor maximo no es uno")
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        with self.assertRaises(ValueError) as cm:  
            result = utils.min_max_scale(arr) 
        self.assertIn("All values are equal; min-max scaling is undefined", str(cm.exception))
